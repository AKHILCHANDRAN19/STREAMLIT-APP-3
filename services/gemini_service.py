...
import logging
import re
from config import get_all_cookies, EXTRA_MCQS, DELAY_BETWEEN_PAGES
...
async def init_gemini_client() -> GeminiClient:
    """Initializes GeminiClient by injecting the full browser cookie jar."""
    psid, full_cookies = get_all_cookies()
    
    # Initialize client with primary PSID
    client = GeminiClient(secure_1psid=psid)
    
    # Inject all remaining cookies (SID, HSID, SSID, COMPASS, NID, etc.)
    for name, val in full_cookies.items():
        client.cookies[name] = val
        
    await client.init(timeout=45, auto_close=False, close_delay=300, auto_refresh=True)
    return client
...
async def generate_mcqs_for_page(chat, page_img: str, page_num: int, doc_type: str = "pointwise", max_retries: int = 3) -> PageMCQOutput:
    ...
    try:
        ...
        logger.warning(f"Counting phase failed on page {page_num}: {e}. Defaulting to target {target_mcqs}")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.warning(f"MCQ generation attempt {attempt} failed on page {page_num}: {e}")
